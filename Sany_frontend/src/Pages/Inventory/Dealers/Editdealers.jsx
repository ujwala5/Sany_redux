...
function Editdealers() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const editDealer = await fetch(`http://localhost:8991/V2/dealers/edit?Dealerid=${dealerId}`, {
                method: 'PUT',
                headers: {
            ...
            const editDealerRes = await editDealer.json();
            console.log("editDealerRes==>", editDealerRes)

            if (editDealerRes.code === 200) {
    ...
    const fetchDealers = async (id) => {
        const fetchDealerRes = await fetch(`http://localhost:8991/V2/showDealerById/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
            }
        ...
        const res = await fetchDealerRes.json();
        // console.log("res==>", res);
        ...
        formik.setFieldValue('dealerCode', result.dealercode);
        formik.setFieldValue('contactNumber', result.contactno);
        formik.setFieldValue('country', result.country);
        formik.setFieldValue('state', result.state);
        formik.setFieldValue('city', result.city);
        formik.setFieldValue('lattitude', result.lat);
        formik.setFieldValue('longitude', result.lon);
        formik.setFieldValue('zipcode', result.zipcode);
        formik.setFieldValue('Address', result.conaddr);
    }
